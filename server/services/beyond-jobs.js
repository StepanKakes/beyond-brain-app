...
const morningBrief = {
  ...
  async run({ log }) {
    ...
    if (!needsUs.length && !risks.length && !todaysCalls.length && !systemic.length) {
    ...
    const result = await runAgent(
      [
        ...
        'a co se dnes děje. Drž Beyond hlas (`system/beyond-hlas.md`). Bez emoji,',
        'bez vaty, maximálně deset řádků. Když je toho málo, napiš málo.',
        '',
...
export const JOBS = [processCall, morningBrief, syncClients, prepareCalls, roadmapCheck, tidyProfiles];
