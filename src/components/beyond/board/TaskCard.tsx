...
export function TaskCard({
  task, person, onOpen, onToggle, onDragStart, onDragEnd, dragging,
}: {
  task: Task;
  /** Whose it is, when the column is not the person (the board by state). */
  person?: { key: string; displayName: string; avatar?: string };
  onOpen: () => void;
  ...
  return (
    <Card onClick={onOpen} dragging={dragging} draggable={!task.virtual} onDragStart={onDragStart} onDragEnd={onDragEnd} className={`bb-tc${done ? ' bb-tc--done' : ''}`}>
      {(task.client || task.dueLabel || person) && (
        <div className="bb-tc__head">
          ...
          {task.dueLabel && (
            <span className={`bb-tc__due${task.dueKind ? ` bb-tc__due--${task.dueKind}` : ''}`}>
              <Calendar size={11} />{task.dueLabel}
            </span>
          )}
          {person && <Avatar name={person.displayName} src={person.avatar} size={18} />}
        </div>
      )}
      ...
      <div className="bb-tc__meta">
        ...
        {task.prep && <Chip icon={<Sparkles size={12} />}>{task.prep.kind === 'zprava' ? 'Zpráva' : 'Podklad'}</Chip>}
      </div>
    </Card>
  );
}
