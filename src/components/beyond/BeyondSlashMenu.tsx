...
export default function BeyondSlashMenu({
  ...
  return (
    <div className="bb-sheet absolute inset-x-2 bottom-full z-40 mb-2 max-h-[300px] overflow-y-auto rounded-[16px] border border-black/[0.07] p-1.5 shadow-lg">
      ...
      {items.map((cmd, i) => (
        <button
          ...
          className={`flex w-full items-center gap-2.5 rounded-[11px] px-2.5 py-2 text-left transition-colors ${
            i === activeIndex ? 'bg-black/[0.06]' : 'hover:bg-black/[0.03]'
          }`}
        >
          ...
          <span className="flex-shrink-0 rounded-full bg-black/[0.04] px-1.5 py-0.5 text-[10px] text-beyond-faint">
